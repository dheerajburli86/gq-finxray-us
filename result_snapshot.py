...
def edgar_fallback_url(ticker, form_type):
    """Fallback when the triggering filing's own EDGAR URL isn't available.

    Lands on EDGAR's filing list for this company scoped to this form type,
    newest first -- one click from the actual document. Deliberately NOT a
    guessed deep link: browse-edgar accepts a plain ticker in the CIK param
    and has done so for years, whereas a constructed archive path would need
    the CIK zero-padded plus the accession number, and getting either wrong
    yields a 404 rather than a degraded-but-working page.
    """
    return (
...
def build_result_snapshot(ticker, form_type, filing_url=None):
    ...
    return {
        ...
        # The 10-Q/10-K that triggered this snapshot IS the filing for this
        # exact period, so its own EDGAR URL is the correct "go to that
        # quarter" target -- no per-quarter deep link needs constructing.
        "link": filing_url or edgar_fallback_url(ticker, form_type),
        "link_is_exact_filing": bool(filing_url),
        "metrics": {
    ...
    try:
        supabase.table("alerts").insert({
            ...
            "extra": tag_extra({
                ...
                "link_is_exact_filing": snapshot.get("link_is_exact_filing", False)
            }, "FMP", "RESULT_SNAPSHOT"),
            "delivered": False
        }).execute()
    ...
    try:
        ...
        for filing in filings:
            ...
            snapshot = build_result_snapshot(
                ticker, form_type, filing_url=filing.get("filing_url")
            )
            if snapshot:
...
if __name__ == "__main__":
    ...
    if snapshot:
        ...
        print(f"Exact filing link: {snapshot['link_is_exact_filing']}")
    else:
