...
#
# BUGFIX 2026-08-19: this was $200,000,000 — a BULK-DEAL notional applied to an
# individual Form 4 line item. Even the largest single insider disposition at a
# megacap is typically $5-50M, so the branch was structurally unreachable and
# BULK_DEAL alerts had never once fired. $10M is genuinely notable for an
# individual officer or director without being routine.
LARGE_TRADE_THRESHOLD_USD = float(os.getenv("LARGE_TRADE_THRESHOLD_USD", "10000000"))
...
def poll_ticker_news(tickers):
    ...
    # FMP happily returns articles for symbols we did not ask about (related
    # names, index constituents, "MARKET" filler). Every one of those became a
    # PENDING raw_filings row that the single-threaded AI pipeline had to look at
    # and then discard. 2,604 of 2,803 pending rows were for tickers nobody
    # watches — that backlog is why watched tickers were taking hours to
    # summarise. Anything not explicitly requested is dropped at ingest now.
    wanted = {str(t).upper() for t in ticker_list}
    ...
    for i in range(0, len(ticker_list), BATCH_SIZE):
        ...
        for article in articles:
            # Reject unrequested symbols BEFORE the dedup cache, so a flood of
            # unwatched articles cannot evict watched URLs from the cache.
    ...
    print(f"[FMP NEWS] stored={total} duplicates={duplicates} "
          f"dropped_unwatched={unwatched} cache={cache_info} URLs")
    return total
...
def poll_insider_transactions(tickers):
    ...
    for ticker in sorted(tickers):
        ...
        for txn in txns:
            ...
            already_have_form4 = insider_already_stored(ticker, txn_date, insider)
            ...
            # BUGFIX 2026-08-19: the large-trade branch below used to sit INSIDE
            # this insert's success path, after a `continue` on the
            # already-stored check. So a qualifying large trade whose routine
            # Form 4 record had been stored on an earlier cycle — which is the
            # normal case, since this poller re-reads a 7-day window — was never
            # evaluated at all. The two checks are independent events and are
            # now evaluated independently.
            if not already_have_form4:
                try:
                    supabase.table("raw_filings").insert({
