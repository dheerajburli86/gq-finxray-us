...
def get_sec_filings(symbol, from_date, to_date, form_type=None, limit=100):
    ...
    each record has `link` (EDGAR index page) and `finalLink` (the document).
    Every other FMP endpoint is data-only, so this is how any alert derived
    from a filing gets an addressable source.
    """
    params = {"symbol": symbol, "from": from_date, "to": to_date,
              "page": 0, "limit": limit}
    data = _get("sec-filings-search/symbol", params)
    if not isinstance(data, list):
        return []
    if form_type:
        data = [f for f in data
                if str(f.get("type", "")).upper() == form_type.upper()]
    return data
