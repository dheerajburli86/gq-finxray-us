...
def verify_run_log():
    ...
    try:
        supabase.table("alert_run_log").insert({
            ...
            "source": "STARTUP",
            "filing_type": BUILD_TAG,
            "telegram_success": False,
            "telegram_error": f"boot probe {et_now():%Y-%m-%d %H:%M:%S ET} "
                              f"build={BUILD_TAG}",
        }).execute()
        print(f"[RUNLOG] Self-test PASSED -- audit writes are landing. "
              f"build={BUILD_TAG}")
        return True
    except Exception as e:
...
def _record_telegram_probe(ok, detail):
    """Write the Telegram self-test verdict into the database.

    The deploy log is the obvious place for this, but in practice the boot
    lines scroll away before anyone reads them and the log view is dominated
    by per-request HTTP noise. Putting the verdict in a row means the question
    'is Telegram working, and if not what exactly did it say?' can be answered
    with one query, by anyone, at any time after the fact.
    """
    try:
        supabase.table("alert_run_log").insert({
            "ticker": "__TG_PROBE__",
            "source": "STARTUP",
            "filing_type": BUILD_TAG,
            "telegram_success": bool(ok),
            "telegram_error": str(detail)[:500],
        }).execute()
    except Exception:
        pass
...
async def verify_telegram():
    ...
    if not TELEGRAM_TOKEN:
        ...
        _record_telegram_probe(False, "TELEGRAM_TOKEN not set")
        return False
    if not TELEGRAM_CHANNEL_ID:
        print("[TELEGRAM] Self-test FAILED -- TELEGRAM_CHANNEL_ID is not set.")
        _record_telegram_probe(False, "TELEGRAM_CHANNEL_ID not set")
        return False
    try:
        async with Bot(token=TELEGRAM_TOKEN) as bot:
            me = await bot.get_me()
        ...
        _record_telegram_probe(True, f"posting as @{me.username} "
                                     f"to chat {TELEGRAM_CHANNEL_ID}")
        return True
    except Exception as e:
        ...
        _record_telegram_probe(False, f"{type(e).__name__}: {e} "
